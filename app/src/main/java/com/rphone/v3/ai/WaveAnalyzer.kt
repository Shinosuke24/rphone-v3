...
/**
 * WaveAnalyzer — Analisa statistik sinyal arus untuk Lapis 2 AI
 */
object WaveAnalyzer {
    ...
    private fun deteksiStuck(wave: List<Float>): StuckInfo {
        ...
        return StuckInfo(
            detected   = true,
            value      = (Math.round(bestValue * 100f) / 100f),
    ...
    private fun deteksiSpike(wave: List<Float>): SpikeInfo {
        ...
        return SpikeInfo(
            detected = count > 0,
            maxDelta = (Math.round(maxDelta * 100f) / 100f),
            count    = count
        )
    }
